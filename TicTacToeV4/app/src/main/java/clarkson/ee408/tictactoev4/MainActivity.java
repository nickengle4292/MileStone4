...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Retrieve the player value from the intent
        int player = getIntent().getIntExtra("player", 1);

        // Pass the player value to the TicTacToe constructor
        tttGame = new TicTacToe(player);

        // Continue with the rest of your code
        buildGuiByCode();

        // Set shouldRequestMove to true in onCreate()
...
//                gridLayout.addView( buttons[row][col], bParams );
    ...
    private class PlayDialog implements DialogInterface.OnClickListener {
        public void onClick( DialogInterface dialog, int id ) {
            if( id == -1 ) /* YES button */ {
                ...
                updateTurnStatus();
                shouldRequestMove = true; // Set shouldRequestMove to true
            }
    ...
    private void updateTurnStatus() {
        if(tttGame.getPlayer() == tttGame.getTurn()) {
            ...
            enableButtons(true);
            // Remove the usage of shouldRequestMove in updateTurnStatus()
        } else {
        ...
        AppExecutors.getInstance().networkIO().execute(() -> {
            ...
            AppExecutors.getInstance().mainThread().execute(() -> {
                if (response == null) {
                    ...
                    // If the game is not active, end the game
                    endGame(response.getMessage());
    ...
    /**
     * End the game when it is not active
     * @param message Message to display in the status text
     */
    ...
    /**
     * Sends an ABORT_GAME request to the server to signify that the user left an ongoing game
     */
    ...
    /**
     * Sends a COMPLETE_GAME request to the server to signify that the user left the game after refusing to play again
     */
